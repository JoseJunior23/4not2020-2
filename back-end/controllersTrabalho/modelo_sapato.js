...
const ModeloSapato = require('../modelsTrabalho/ModeloSapato')
...
controller.novo = async (req, res) => {
    ...
    try {
        await ModeloSapato.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch(erro) {
...
controller.listar = async (req, res) => {
    try {
        let dados = await ModeloSapato.find() // Traz todos os cursos cadastrados
...
controller.obterUm = async (req, res) => {
    try {
        ...
        let obj = await ModeloSapato.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        let ret = await ModeloSapato.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        ...
        let ret = await ModeloSapato.findByIdAndDelete(id)
